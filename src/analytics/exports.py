...
from src.analytics.cashflow_kpis import _sign, classify_allocation
...
def export_ratio_edge_cases(db_path="db/nifty100.db", output_path=None):
    ...
    latest = pd.read_sql_query(
        text(
            "SELECT fr.company_id, fr.year, fr.return_on_equity_pct, "
            ...
            anomalies.append(
                {
                    ...
                    "category": _categorise(sval, calc),
                }
            )
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"{'TICKER':<14}{'METRIC':<6}{'COMPUTED':>10}{'SOURCE':>10}{'DIFF%':>9}  CATEGORY\n"
        )
        f.write("-" * 72 + "\n")
        f.writelines(
            f"{a['ticker']:<14}{a['metric']:<6}{a['computed']:>10}{a['source']:>10}"
