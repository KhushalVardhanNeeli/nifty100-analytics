...
class RatioEngine:
            ...
            rows.append(
                {
                    ...
                    "return_on_capital_employed_pct": return_on_capital_employed(
                        ebit, equity_cap, reserves, borrowings
                    ),
    ...
    def _merge_cashflow(self, base: pd.DataFrame) -> pd.DataFrame:
        ...
        if not base.empty:
            cfo = pd.read_sql_query(
                text("SELECT company_id, year, operating_activity FROM cashflow"),
                self.engine,
            )
            base = base.merge(
                cfo.rename(columns={"operating_activity": "cash_from_operations_cr"}),
                on=["company_id", "year"],
                how="left",
            )
    ...
    def _store(self, df: pd.DataFrame) -> None:
        ...
        cf = pd.read_sql_query(
            text("SELECT company_id, year, investing_activity FROM cashflow"),
            self.engine,
        )
