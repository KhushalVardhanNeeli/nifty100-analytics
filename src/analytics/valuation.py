...
def compute_valuation(db_path: str = DB_PATH) -> pd.DataFrame:
    ...
    df = mc_latest.merge(companies, on="company_id", how="left").merge(
        fr, on=["company_id", "year"], how="left"
    )
    ...
    five_yr = mc_all.groupby("company_id")["pe_ratio"].median().rename("five_yr_median_PE")
    ...
    df["PE_vs_sector_median_pct"] = df.apply(
        lambda r: (
            (r["pe_ratio"] / r["sector_median_PE"] * 100)
