...
DEFAULTS = {"roe": 0, "de": 5, "fcf": -1000, "rev": -50, "pat": -50,
            "opm": -50, "pe": 200, "pb": 30, "dy": 0, "icr": 0}
...
fcf = st.sidebar.slider("FCF min (Cr)", -2000.0, 10000.0, float(st.session_state.get("fcf", DEFAULTS["fcf"])))
rev = st.sidebar.slider("Revenue CAGR 5y min %", -50.0, 60.0, float(st.session_state.get("rev", DEFAULTS["rev"])))
pat = st.sidebar.slider("PAT CAGR 5y min %", -50.0, 60.0, float(st.session_state.get("pat", DEFAULTS["pat"])))
opm = st.sidebar.slider("OPM min %", -50.0, 60.0, float(st.session_state.get("opm", DEFAULTS["opm"])))
...
dy = st.sidebar.slider("Dividend Yield min %", 0.0, 6.0, float(st.session_state.get("dy", DEFAULTS["dy"])))
icr = st.sidebar.slider("Interest Coverage min", 0.0, 15.0, float(st.session_state.get("icr", DEFAULTS["icr"])))
...
result = df[mask].copy()
result = result.sort_values("composite_score", ascending=False) if "composite_score" in result else result

cols = ["ticker", "company_name", "broad_sector", "roe", "roce", "net_profit_margin",
        "debt_to_equity", "interest_coverage", "free_cash_flow", "revenue_cagr_5y",
        "pat_cagr_5y", "pe_ratio", "pb_ratio", "dividend_yield", "composite_score"]
cols = [c for c in cols if c in result.columns]

st.markdown(f"### {len(result)} companies match your filters")
st.dataframe(result[cols].round(2) if not result.empty else result, use_container_width=True, hide_index=True)
