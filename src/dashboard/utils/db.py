...
def get_sectors() -> list:
    df = _query("SELECT DISTINCT broad_sector FROM companies "
...
def resolve_ticker(ticker: str):
    ...
    if not t:
        return None
    df = _query("SELECT company_id, ticker, company_name FROM companies WHERE upper(ticker) = ?",
                params=[t])
...
def get_ratios(ticker: str, year: int | None = None) -> pd.DataFrame:
    ...
    if cid is None:
        return pd.DataFrame()
    if year is None:
        return _query("SELECT * FROM financial_ratios WHERE company_id = ? ORDER BY year",
                      params=[cid])
    return _query("SELECT * FROM financial_ratios WHERE company_id = ? AND year = ? ORDER BY year",
                  params=[cid, int(year)])
...
def get_pl(ticker: str) -> pd.DataFrame:
    ...
    if cid is None:
        return pd.DataFrame()
    return _query("SELECT * FROM profitandloss WHERE company_id = ? ORDER BY year",
                  params=[cid])
...
def get_bs(ticker: str) -> pd.DataFrame:
    ...
    if cid is None:
        return pd.DataFrame()
    return _query("SELECT * FROM balancesheet WHERE company_id = ? ORDER BY year",
                  params=[cid])
...
def get_cf(ticker: str) -> pd.DataFrame:
    ...
    if cid is None:
        return pd.DataFrame()
    return _query("SELECT * FROM cashflow WHERE company_id = ? ORDER BY year",
                  params=[cid])
...
def get_valuation(ticker: str) -> pd.DataFrame:
    ...
    if cid is None:
        return pd.DataFrame()
    return _query("SELECT * FROM market_cap WHERE company_id = ? ORDER BY year",
                  params=[cid])
...
def get_peers(group_name: str) -> pd.DataFrame:
    return _query(
        """SELECT pg.company_id, c.ticker, c.company_name, pg.is_benchmark
           ...
           WHERE pg.peer_group_name = ? ORDER BY c.ticker""", params=[group_name])
...
def get_peer_percentiles(company_id: int) -> pd.DataFrame:
    return _query(
        ...
        "WHERE company_id = ? ORDER BY metric", params=[int(company_id)])


@st.cache_data(ttl=600)
def get_proscons(company_id: int) -> pd.DataFrame:
    return _query("SELECT pros, cons FROM prosandcons WHERE company_id = ?",
                  params=[int(company_id)])


@st.cache_data(ttl=600)
def get_documents(company_id: int) -> pd.DataFrame:
    return _query("SELECT year, annual_report FROM documents WHERE company_id = ? ORDER BY year DESC",
                  params=[int(company_id)])
