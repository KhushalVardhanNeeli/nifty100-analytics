...
class ProsConsGenerator:
    ...
    def _load(self, db_path: str) -> dict:
        ...
        try:
            ...
            fr = pd.read_sql(
                text("SELECT * FROM financial_ratios ORDER BY company_id, year"), engine
            )
            pl = pd.read_sql(
                text(
            ...
            mc = pd.read_sql(
                text("SELECT company_id, year, dividend_yield_pct " "FROM market_cap"),
                engine,
            )
        finally:
            engine.dispose()
    ...
    def _pro_rules(self, m):
        ...
        if (
            m["roe"] is not None
            and m["roe"] > 20
            and len(m["roe_series"]) >= 3
            and all(r > 20 for r in m["roe_series"][-3:])
        ):
            out.append(
                (
                    1,
        ...
        if (
            m["div_yield"] is not None
            and m["div_yield"] > 2
            and m["fcf"] is not None
            and m["fcf"] > 0
        ):
            out.append(
                (
        ...
        if (
            len(m["roe_series"]) >= 4
            and m["roe_series"][-1] > m["roe_series"][-2] > m["roe_series"][-3]
        ):
            out.append(
                (
                    10,
        ...
        if (
            m["rev_cagr5"] is not None
            and m["pat_cagr5"] is not None
            and m["rev_cagr5"] < m["pat_cagr5"]
        ):
            out.append(
                (
                    11,
    ...
    def _con_rules(self, m):
        ...
        if m["de"] is not None and m["de"] > 2 and not m["is_fin"]:
            out.append(
                (
                    1,
                    f"Debt-to-equity ratio of {m['de']:.2f} is elevated for a non-financial company and warrants monitoring",
        ...
        if (
            len(m["sales_series"]) >= 3
            and m["sales_series"][-1] < m["sales_series"][-2] < m["sales_series"][-3]
        ):
            out.append(
                (
                    5,
        ...
        if (
            len(m["de_series"]) >= 4
            and m["de_series"][-1] > m["de_series"][-2] > m["de_series"][-3] > m["de_series"][-4]
        ):
            out.append(
                (
                    8,
    ...
    def compute(self, db_path: str = DB_PATH) -> pd.DataFrame:
        ...
        print(
            f"[NLP] Generated {len(df)} pros/cons rows for {df['company_id'].nunique()} companies"
        )
        return df
