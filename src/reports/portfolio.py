...
def build_portfolio(db_path: str = "db/nifty100.db", output_dir: str = REPORTS_DIR) -> str:
    ...
    fr = pd.read_sql(
        text("SELECT * FROM financial_ratios WHERE year IN (:a, :b)"),
        engine,
        params={"a": latest, "b": latest - 1},
    )
    engine.dispose()
    ...
    doc = SimpleDocTemplate(
        f"{output_dir}/portfolio_summary.pdf",
        pagesize=A4,
        leftMargin=14 * mm,
        rightMargin=14 * mm,
        topMargin=12 * mm,
    ...
    story = [
        Paragraph("Nifty 100 Portfolio Summary", h1),
        Paragraph(f"Latest year: {latest}", body),
        Spacer(1, 10),
    ]
    ...
    for _, c in companies.iterrows():
        ...
        head = Table(
            [[Paragraph(f"{c['company_name']} ({c['ticker']})", h2)]],
            colWidths=[182 * mm],
        )
        head.setStyle(
            TableStyle(
        ...
        for label, col in KPIS:
            ...
            rows.append(
                [
                    label,
                    ("N/A" if cur_v is None or pd.isna(cur_v) else f"{float(cur_v):,.1f}"),
