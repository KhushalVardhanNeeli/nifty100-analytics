...
def build_sector_report(sector: str, db_path: str = "db/nifty100.db",
    ...
    companies = pd.read_sql(text("SELECT company_id, ticker, company_name, broad_sector FROM companies"),
                            engine)
    fr = pd.read_sql(text("SELECT * FROM financial_ratios WHERE year = :y"), engine, params={"y": latest})
    engine.dispose()
    ...
    doc = SimpleDocTemplate(f"{output_dir}/{fname}_report.pdf", pagesize=landscape(A4),
                            leftMargin=12 * mm, rightMargin=12 * mm,
                            topMargin=12 * mm, bottomMargin=12 * mm)
    h1 = ParagraphStyle("h1", fontName="Helvetica-Bold", fontSize=18, textColor=colors.HexColor("#1F3864"))
    ...
    for _, r in data.sort_values("ticker").iterrows():
        rows.append([r["ticker"], r["company_name"]] +
                    [_fmt(r[col]) for _, col in METRICS])
...
def generate_all(db_path: str = "db/nifty100.db", output_dir: str = REPORTS_DIR):
    engine = create_engine(f"sqlite:///{db_path}")
    sectors = pd.read_sql(text("SELECT DISTINCT broad_sector FROM companies "
                               "WHERE broad_sector IS NOT NULL"), engine)["broad_sector"].tolist()
    engine.dispose()
