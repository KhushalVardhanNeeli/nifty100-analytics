...
def _load_company(engine, cid):
    comp = pd.read_sql(text("SELECT * FROM companies WHERE company_id = :c"), engine, params={"c": cid})
    if comp.empty:
        return None
    comp = comp.iloc[0]
    fr = pd.read_sql(text("SELECT * FROM financial_ratios WHERE company_id = :c ORDER BY year"),
                     engine, params={"c": cid})
    pl = pd.read_sql(text("SELECT * FROM profitandloss WHERE company_id = :c ORDER BY year"),
                     engine, params={"c": cid})
    bs = pd.read_sql(text("SELECT * FROM balancesheet WHERE company_id = :c ORDER BY year"),
                     engine, params={"c": cid})
    cf = pd.read_sql(text("SELECT * FROM cashflow WHERE company_id = :c ORDER BY year"),
                     engine, params={"c": cid})
...
def revenue_profit_chart(pl, path):
    ...
    ax.bar([i + 0.2 for i in x], d["net_profit"], width=0.4, label="Net Profit", color="#70AD47")
    ax.set_xticks(list(x)); ax.set_xticklabels(d["year"], rotation=45, fontsize=7)
    ax.legend(fontsize=7); ax.set_title("Revenue and Net Profit", fontsize=10)
    fig.tight_layout(); return _chart_png(fig, path)
...
def roe_roce_chart(fr, path):
    ...
    ax2 = ax.twinx()
    ax2.plot(fr["year"], fr["return_on_capital_employed_pct"], "s--", label="ROCE %", color="#C55A11")
    ax.set_xlabel("Year", fontsize=8); ax.set_ylabel("ROE %", fontsize=8); ax2.set_ylabel("ROCE %", fontsize=8)
...
def bs_composition_chart(bs, path):
    ...
    ax.bar(d["year"], d["borrowings"], bottom=d["equity_capital"], label="Borrowings", color="#ED7D31")
    ax.bar(d["year"], d["other_liabilities"],
           bottom=d["equity_capital"] + d["borrowings"], label="Other Liab.", color="#A5A5A5")
    ax.legend(fontsize=7); ax.set_title("Balance Sheet Composition", fontsize=10)
...
def cf_waterfall_chart(cf, path):
    ...
    vals = [r.get("operating_activity"), r.get("investing_activity"),
...
def _style():
    return {
        ...
        "pro": ParagraphStyle("pro", fontName="Helvetica", fontSize=9, textColor=GREEN,
                              leftIndent=8, bulletIndent=0, spaceAfter=2),
        "con": ParagraphStyle("con", fontName="Helvetica", fontSize=9, textColor=RED,
                              leftIndent=8, bulletIndent=0, spaceAfter=2),
    }
...
def build_tearsheet(company_id: int, db_path: str = "db/nifty100.db",
    ...
    doc = SimpleDocTemplate(f"{output_dir}/{ticker}_tearsheet.pdf", pagesize=A4,
                            leftMargin=14 * mm, rightMargin=14 * mm,
                            topMargin=12 * mm, bottomMargin=12 * mm)
    story = []

    # ── Page 1 ──────────────────────────────────────────────────────
    header = Table([[Paragraph(f"{comp['company_name']} ({ticker})", S["h1"])],
                    [Paragraph(f"Sector: {comp['broad_sector']} · Sub-sector: {comp['sub_sector']}",
                               S["sub"])]],
                   colWidths=[182 * mm])
    ...
    kpis = [
        ...
        ("ROCE %", _fmt(r["return_on_capital_employed_pct"] if r is not None else None)),
        ("Net Profit Margin %", _fmt(r["net_profit_margin_pct"] if r is not None else None)),
    ...
    charts = Table([[Image(p1, width=88 * mm, height=48 * mm),
    ...
    chart2 = Table([[Image(p3, width=88 * mm, height=46 * mm),
                     Image(p4, width=88 * mm, height=46 * mm)]])
    chart2.setStyle(TableStyle([("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(chart2)
    story.append(Spacer(1, 8))

    story.append(Paragraph(f"Capital Allocation: {r['capital_allocation_pattern'] or 'N/A'}", S["h2"]))
...
def generate_all(db_path: str = "db/nifty100.db", output_dir: str = REPORTS_DIR):
    engine = create_engine(f"sqlite:///{db_path}")
    counts = pd.read_sql(
        text("SELECT company_id, COUNT(*) AS n FROM profitandloss GROUP BY company_id"), engine)
    engine.dispose()
    ...
    for _, row in counts.iterrows():
        if row["n"] < 3:
            skipped.append({"company_id": int(row["company_id"]), "reason": f"Only {row['n']} years"})
            continue
        try:
