...
class ScreenerEngine:
    ...
    OUTPUT_COLS = [
        "company_id", "ticker", "company_name", "broad_sector",
        "roe", "roce", "net_profit_margin", "operating_profit_margin",
        "debt_to_equity", "interest_coverage", "free_cash_flow",
        "revenue_cagr_3y", "revenue_cagr_5y", "pat_cagr_5y", "eps_cagr_5y",
        "pe_ratio", "pb_ratio", "dividend_yield", "market_cap",
        "net_profit", "sales", "composite_score",
    ]
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        try:
            latest = int(pd.read_sql("SELECT MAX(year) AS y FROM financial_ratios", conn).iloc[0]["y"])
            ...
            prev = pd.read_sql(
                ...
                conn, params=[int(latest) - 1])
            df = df.merge(prev.rename(columns={"debt_to_equity": "de_prev"}),
                          on="company_id", how="left")
            df["de_declining"] = df.apply(
                lambda r: (pd.notna(r.get("debt_to_equity")) and pd.notna(r.get("de_prev"))
                           and r["debt_to_equity"] < r["de_prev"]), axis=1)

            # FCF 5-year CAGR from cashflow
            cf = pd.read_sql(
                "SELECT company_id, year, operating_activity, investing_activity "
                "FROM cashflow ORDER BY company_id, year", conn)
    ...
    def composite_score(self, df) -> pd.DataFrame:
        ...
        fcf_pos = (result.get("free_cash_flow", pd.Series(0, index=result.index)).fillna(0) > 0) * 100.0
    ...
    def screen(self, preset_name) -> pd.DataFrame:
        ...
        return scored[self.OUTPUT_COLS + ["composite_score_sector"] if "composite_score_sector" in scored.columns
        ...
        for ridx, (_, row) in enumerate(df.iterrows(), 2):
            for cidx, cname in enumerate(cols, 1):
                ...
                if isinstance(val, float) and (pd.isna(val) or np.isnan(val)):
                    val = None
                cell = ws.cell(row=ridx, column=cidx, value=round(val, 2) if isinstance(val, float) else val)
